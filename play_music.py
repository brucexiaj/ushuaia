...
class mainFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        self.SetBackgroundColour((43, 43, 43))
        ...
        # 开启线程监控当前播放的歌曲，结束后自动播放顺序的下一首
        monitor_music_play_thread = Thread(target=self.monitor_music_play)
        monitor_music_play_thread.start()

    def monitor_music_play(self):
        time.sleep(1)  # 这里如果不sleep，pygame.init()会立刻执行，界面会出错
        pygame.init()  # 没有这句代码，pygame.event.get()会报错
        while True:
            for event in pygame.event.get():
                if event.type == self.SONG_FINISHED:
                    self.play_next_music(None)
                    time.sleep(1)
    ...
    def draw_navi_panel(self):
        ...
        local_music_text.SetOwnForegroundColour((255, 255, 255))
    ...
    def draw_music_list_panel(self):
        ...
        for music_index in range(local_music_num):
            music_text = wx.StaticText(self.music_list_panel, -1, self.local_music_name_list[music_index],
                                       pos=(0, music_index * 40 + 20), size=(270, 30), style=wx.ALIGN_RIGHT)
            music_text.SetOwnForegroundColour((255, 255, 255))
    ...
    def draw_play_music_panel(self):
        ...
        self.current_music_static_text.SetOwnForegroundColour((255, 255, 255))
    ...
    def redraw_music_lyric_panel(self, start_index=0):
        ...
        for lyric_index in range(start_index, start_index + MAX_LYRIC_ROW, 1):
            if lyric_index < len(lyric_list):
                ...
                self.lyrcis_static_text[lyric_relative_index].SetOwnForegroundColour((255, 255, 255))
    ...
    def play_music(self):
        ...
        self.music.set_endevent(self.SONG_FINISHED)
        # step2：重写歌词面板
        ...
        # 更改当前播放的音乐的名字
        self.current_music_static_text.SetLabelText(self.local_music_name_list[self.current_music_index].replace(".mp3", ""))
    ...
    def refersh_lyrics(self):
        ...
        while self.music.get_busy():  # 播放中
            ...
            for lyric_index, lyrics_time_dict in enumerate(lyrics_time_dict_list):
                ...
                if math.fabs(lyric_time - current_time) < 0.5:
